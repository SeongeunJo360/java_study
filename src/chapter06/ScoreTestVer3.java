 ...
 * - 1차원 배열로 학생 정보를 저장한다.
 * - 학생은 홍길동, 이순신, 김유신, 강감찬, 홍길순 nameList
 * - 과목은 국어, 영어, 수학 3과목의 점수를 입력 subjectList, scoreList
 * - 입력받은 과목의 총점과 평균을 구함 tot, avg
 * - 학생명, 과목별 점수, 총평, 평균은 각각 1차원 배열로 생성하여 관리
	...
	public static void main(String[] args) {
		...
		while(menuFlag) {
			...
			if(menu == 1) {//학생 등록
				...
				//1.수정할 학생명의 존재여부 검색: 무-> 검색 데이터 존재X, 반복진행
				if(count != 0) {
					...
					while(deleteFlag) {
						...
						if (deleteIdx != -1) {
							for(int i=deleteIdx;i<count-1;i++) {//홍길동(0) 이순신(1) 김유신(2)
																//이순신(0) 김유신(1) 김유신(2)-새 데이터가 들어올 위치
